# 📡 NEXUS-ZERO: The Living Infrastructure
**World's first AI-orchestrated disaster mesh network.**

## 👥 The Team (SRM T-REX 2026)
* **L. MOHAMMED RAZAQ [Lead]** - System Architect
* **S. NITHYA** - AI/ML Specialist 
* **SS. MONISH KUMAR** - Network Protocols
* **K.A. DHARAN BALAJI** - Security & Pulse Cryptography
* **P. NIVETHA** - Spatial UX & Design
* P.K. YUVA PRIYAN: Embedded Systems & Hardware Gateway Integration

## 🛠️ Tech Stack
- **Languages:** Python, C++
- **AI Models:** Google Lyra (Audio), Gemini Nano (Triage)
- **Protocols:** BLE, WiFi-Direct, RSSI Fingerprinting

## 🏆 Innovation Points
- **Neural Voice:** 3kbps calls through 10+ offline hops.
- **Shadow-GPS:** Satellite-free location via WiFi MAC-IDs.
- **Bio-Audit:** Heartbeat-verified identity to stop panic/fake news.
